...
module PlaceOS::Core::Api
  abstract class Application < ActionController::Base
    ...
    def bad_media_type(error) : ContentError
      ContentError.new error: error.message.to_s, accepts: error.accepts
    end
    ...
    def invalid_param(error) : ParameterError
      ParameterError.new error: error.message.to_s, parameter: error.parameter, restriction: error.restriction
    end
  end
end
